...
describe("KitchenService", () => {
    ...
    it("should fetch job by id", async () => {
        ...
        prisma.job.findUnique.mockResolvedValue(fakeJob);
    ...
    it("should create job, publish event and cache it", async () => {
        const fakeJob = { id: jobId, orderId, status: "ACCEPTED" };
        prisma.job.create.mockResolvedValue(fakeJob);
        const result = await KitchenService.createJob(userId, orderId);
        expect(prisma.job.create).toHaveBeenCalledWith({
            data: { orderId, status: "ACCEPTED" },
        });
        expect(mq.publish).toHaveBeenCalledWith("kitchen.job.accepted", { orderId, userId });
        expect(redis.set).toHaveBeenCalledWith(`job:${jobId}`, JSON.stringify(fakeJob));
        expect(result).toEqual(fakeJob);
    });
    /* =====================================
       rejectJob
    ...
    it("should reject job and publish event", async () => {
        ...
        prisma.job.update.mockResolvedValue(fakeJob);
        ...
        expect(redis.set).toHaveBeenCalledWith(`job:${jobId}`, JSON.stringify(fakeJob));
        expect(mq.publish).toHaveBeenCalledWith("kitchen.job.rejected", { orderId });
        expect(result).toEqual(fakeJob);
    });
    /* =====================================
       startJob
    ...
    it("should move job to IN_PROGRESS from cache", async () => {
        ...
        redis.get.mockResolvedValue(JSON.stringify(cachedJob));
        const result = await KitchenService.startJob(jobId);
        expect(redis.get).toHaveBeenCalledWith(`job:${jobId}`);
        expect(redis.set).toHaveBeenCalledWith(`job:${jobId}`, JSON.stringify({
            ...cachedJob,
            status: "IN_PROGRESS",
        }));
    ...
    it("should throw if job not found in cache", async () => {
        ...
        await expect(KitchenService.startJob(jobId)).rejects.toThrow("Job not found in cache");
    });
    /* =====================================
       markCompleted
       ===================================== */
    it("should mark job completed and publish event", async () => {
        ...
        expect(redis.set).toHaveBeenCalledWith(`job:${jobId}`, JSON.stringify(fakeJob));
        expect(mq.publish).toHaveBeenCalledWith("kitchen.job.completed", { orderId });
        expect(result).toEqual(fakeJob);
    });
    /* =====================================
       markFailed
    ...
    it("should mark job failed and publish event with reason", async () => {
        ...
        expect(redis.set).toHaveBeenCalledWith(`job:${jobId}`, JSON.stringify(fakeJob));
        expect(mq.publish).toHaveBeenCalledWith("kitchen.job.failed", {
            orderId,
            reason: "Burnt",
        });
        expect(result).toEqual(fakeJob);
