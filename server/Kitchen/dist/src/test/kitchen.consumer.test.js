...
describe("KitchenConsumer", () => {
    ...
    beforeEach(() => {
        ...
        mq.subscribe.mockImplementation(async (_queue, _event, callback) => {
    ...
    it("should subscribe to inventory.order.queue", async () => {
        ...
        expect(mq.subscribe).toHaveBeenCalledWith("inventory.order.queue", "order.created", expect.any(Function));
    });
    /* =========================================
       SUCCESS FLOW
       ========================================= */
    it("should create job and mark completed after timeout", async () => {
        KitchenService.createJob.mockResolvedValue({
            id: jobId,
            orderId,
        });
        KitchenService.markCompleted.mockResolvedValue({});
        await KitchenConsumer();
        // simulate incoming MQ message
        await subscribedCallback({ orderId, userId });
        expect(KitchenService.createJob).toHaveBeenCalledWith(userId, orderId);
        // Fast-forward timers
        jest.runAllTimers();
        // Wait for async inside setTimeout
        await Promise.resolve();
        expect(KitchenService.markCompleted).toHaveBeenCalledWith(jobId);
    });
    /* =========================================
       FAILURE FLOW
       ========================================= */
    it("should call markFailed if markCompleted throws", async () => {
        ...
        KitchenService.markCompleted.mockRejectedValue(new Error("DB ERROR"));
        KitchenService.markFailed.mockResolvedValue({});
        await KitchenConsumer();
        await subscribedCallback({ orderId, userId });
        jest.runAllTimers();
        await Promise.resolve();
        expect(KitchenService.markCompleted).toHaveBeenCalled();
        expect(KitchenService.markFailed).toHaveBeenCalledWith(jobId, "DB ERROR");
    });
    /* =========================================
       EMPTY MESSAGE
       ========================================= */
